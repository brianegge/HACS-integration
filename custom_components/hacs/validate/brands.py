...
class Validator(ActionValidationBase):
    ...
    async def async_validate(self) -> None:
        """Validate the repository."""
        icon_path = f"{self.repository.content.path.remote}/brand/icon.png"
        if icon_path in (x.full_path for x in self.repository.tree):
            return
        ...
        if self.repository.data.domain not in content["custom"]:
            raise ValidationException(
                "The repository has no brand icons (expected a "
                "brand/icon.png file or domain added to the brands repo)"
            )
